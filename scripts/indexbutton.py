...
gallery_dirs = ["galleries", "galleries_by_tag"]  # Ordner, in denen Seiten aktualisiert werden
...
# --- Alle Buttons einmal global erzeugen ---
tag_files = sorted(
...
new_buttons_block = [
    f"{button_prefix}{fname.replace('tag-', '').replace('.md', '').replace('-', ' ').title()} <{tag_gallery_dir}/{fname}>{button_suffix}\n"
...
# --- 2. Alle .md in galleries/ und galleries_by_tag/ aktualisieren ---
